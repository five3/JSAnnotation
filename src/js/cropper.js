...
var handlers = {
  ...
  cropStart: function cropStart(e) {
    ...
    if(action === 'crop'){   //新创建crop
      ...
      var cropBoxName = (new Date()).getTime();
      $cropBox.attr('name', cropBoxName);
  ...
  cropEnd: function cropEnd(e) {
    ...
    if (originalEvent && originalEvent.changedTouches) {
      $.each(originalEvent.changedTouches, function (i, touch) {
        ...
        var oldPointer = oldPointers[touch.identifier];
        var range = {
          x : pointer.startX - oldPointer.startX,
          y : pointer.startY - oldPointer.startY
        };
        ranges[touch.identifier] = range;
        delete pointers[touch.identifier];
      });
    } else {
      var pointer = pointers[originalEvent && originalEvent.pointerId || 0];
      var oldPointer = oldPointers[originalEvent && originalEvent.pointerId || 0] || {};
      ...
      ranges[originalEvent && originalEvent.pointerId || 0] = range;
    ...
    if($target.hasClass('cropper-face')){  //double click
      var $cropBox = self.$cropBox;
      $cropBox.remove();
      self.$cropBox = null;
      return;
    ...
    if(action === 'crop'){  //new
      ...
      $.each(cropBoxs, function(k, v){
        $cropBox = $('.cropper-crop-box[name='+k+']');
        self.$cropBox = $cropBox;
        v.left += ranges["1"].x;
        v.top += ranges["1"].y;
        self.setCropBoxData(v);
      });
      return;
    }else{ //resize
...
var methods = {
  ...
  // Set the crop label for current cropbox
  setLabel: function setLabel(labelTxt) {
    var self = this;
    var $cropBox = self.$cropBox;
    var cropBoxs = self.cropBoxs;
    ...
    $cropBox.attr('label', labelTxt);

    var name = $cropBox.attr('name');
    var cropBox = cropBoxs[name];
    cropBox.label = labelTxt;
  },
  ...
  // Get the crop label for current cropbox
  ...
  clear: function clear() {
    ...
    self.$container.find('.cropper-crop-box').remove();
    self.cropBoxs = {};
  ...
  zoomTo: function zoomTo(ratio, _event) {
    ...
    //get all crop data
    $.each(cropBoxs, function(k, v){
      self.cropBox = cropBoxs[k];
      cropData[k] = self.getData();
    });
    ...
    if (ratio >= 0 && self.ready && !self.disabled && options.zoomable) {
      ...
      //reset all crop data
      $.each(cropData, function(k, v){
        self.$cropBox = $('.cropper-crop-box[name='+k+']');
        self.setData(cropData[k]);
      });
    }
  },
...
var Cropper = function () {
  function Cropper(element, options) {
    ...
    self.$element = $(element); //原始img或canvas
    self.options = $.extend({}, DEFAULTS, $.isPlainObject(options) && options); //传递的参数
    self.loaded = false;
    self.ready = false;
    ...
    self.limited = false;
    self.wheeling = false;
    self.isImg = false;  //原始节点是 img 或 canvas节点
    self.originalUrl = ''; //原始图片的url
    self.canvas = null;   //当前canvas
    self.cropBox = null;  //当前裁剪框坐标数据
    self.cropBoxs = {};  //所有的裁剪框坐标数据， 新增
    self.pointers = {};  //当前裁剪框的坐标数据
    self.allPointers = {};  //所有裁剪框的坐标数据， 新增
    self.init();
  }
  ...
  createClass(Cropper, [{
    ...
    value: function init() {
      var self = this;
      var $this = self.$element;  // $this ==> 原始img或canvas
      var url = void 0;
      ...
      if ($this.is('img')) {
        ...
        self.originalUrl = url = $this.attr('src');  //原始图片的src值，即用户填写的实际值，可能是一个相对路径
    ...
    value: function load(url) {
      var self = this;
      var options = self.options; //用户参数
      var $this = self.$element; //原始图片对象
    ...
    value: function read(arrayBuffer) {
      debugger; //can not in by use local image file
      //To re-rotate the picture for JPEG type if have exif info
      var self = this;
      var options = self.options;
    ...
    value: function clone() {
      ...
      if (self.isImg) {
        if ($this[0].complete) {
          ...
          $this.one('load', $.proxy(self.start, this));//注册单次事件
        }
      } else {//加载DATAURL到img标签
    ...
    value: function start() {
      var self = this;
      var $clone = self.$clone;  //复制的img节点
      var $image = self.$element; //原始img节点
    ...
    value: function build() {
      var self = this;
      var options = self.options;
      var $this = self.$element;//原始的img节点
      var $clone = self.$clone; //复制的img节点
      var $cropper = void 0; //cropper container
      var $cropBox = void 0; //裁剪框
      var $face = void 0;
      ...
      self.$container = $this.parent(); //原图片父类
      self.$cropper = $cropper = $(TEMPLATE); //cropper container html
      self.$canvas = $cropper.find('.cropper-canvas').append($clone);  //croper canvas
